...
#zero_counts <- colSums(dtm_df == 0)
# Find columns with zero counts exceeding the threshold
#filtered_counts <- row.names(data.frame(zero_counts[zero_counts > 0.9 * nrow(dtm_df)]))
#filtered_counts <- c(filtered_counts, "translation_missing_en_hosting_amenity_49","translation_missing_en_hosting_amenity_50")
#dtm_df <- dtm_df[, !(colnames(dtm_df) %in% filtered_counts)]
...
#names(train_RF_perfect)[1] <- "perfect_rating_score"
